...
function PermissionApprovals() {
  ...
  // Modal State
  const [actionModal, setActionModal] = useState<{ isOpen: boolean; request: any; action: 'Approved' | 'Rejected' | null }>({
    isOpen: false,
    request: null,
    action: null
  });
  ...
  const fetchRequests = async (status: string) => {
    try {
        const res = await api.get(`/permissions/all?status=${status === 'All History' ? 'All' : status}`);
  ...
  const handleConfirmAction = async () => {
    ...
    try {
        ...
        setMsg({ type: 'success', text: `Request ${actionModal.action} successfully` });
  ...
  const columns = [
    ...
    { header: 'Date', accessorKey: 'date', cell: (row: any) => formatDate(row.date) },
    ...
    <div className="p-8 space-y-8">
       ...
       <RNCard>
          <RNTabs 
            ...
            tabs={[
                { id: 'Pending', label: 'Pending Requests', content: <div className="pt-4"><RNTable data={requests} columns={columns} /></div> },
                { id: 'Approved', label: 'Approved', content: <div className="pt-4"><RNTable data={requests} columns={columns} /></div> },
                { id: 'Rejected', label: 'Rejected', content: <div className="pt-4"><RNTable data={requests} columns={columns} /></div> },
                { id: 'All History', label: 'All History', content: <div className="pt-4"><RNTable data={requests} columns={columns} /></div> },
            ]}
          />
       </RNCard>
            ...
            <div className={`bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 border-t-4 ${actionModal.action === 'Rejected' ? 'border-red-500' : 'border-green-500'}`}>
                <div className="p-6">
                    <h3 className="text-xl font-bold mb-2">
                        {actionModal.action === 'Rejected' ? 'Reject Request' : 'Approve Request'}
                    </h3>
                    <p className="text-gray-500 mb-6">
                        You are about to {actionModal.action === 'Rejected' ? 'reject' : 'approve'} the request for <span className="font-semibold text-gray-800">{actionModal.request.userName}</span>.
                    </p>
