...
function UserManagement() {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessorKey: "role",
      cell: (row: any) => (
        <RNBadge variant={row.role === "Admin" ? "destructive" : "secondary"}>
          {row.role}
        </RNBadge>
      ),
    },
        ...
        <div className="flex gap-2">
          {row.status === "Invited" && row.inviteLink && (
            <RNButton
              ...
              onClick={() => copyLink(row.inviteLink)}
            >
              Copy Invite
            </RNButton>
          )}
    ...
    <div className="p-8 space-y-8 relative">
          ...
          {inviteLink && (
            <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 bg-white dark:bg-[var(--color-surface-800)] p-2 rounded border border-[var(--border-default)] text-xs text-[var(--text-primary)]">
                  {inviteLink}
                </code>
                <RNButton
                  size="xs"
      ...
      {editingBalanceUser && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <RNCard className="w-full max-w-md bg-[var(--bg-card)]">
            <h2 className="text-xl font-bold mb-4 text-[var(--text-primary)]">
              Manage Leave Balance: {editingBalanceUser.name}
            </h2>
            <div className="space-y-4">
              <RNInput
                ...
                value={balances.sick}
                onChange={(e) =>
                  setBalances({ ...balances, sick: e.target.value as any })
                }
              />
              <RNInput
                ...
                onChange={(e) =>
                  setBalances({ ...balances, casual: e.target.value as any })
                }
              />
              <RNInput
                label="Earned Leave Quota"
                type="number"
                value={balances.earned}
                onChange={(e) =>
                  setBalances({ ...balances, earned: e.target.value as any })
                }
              />
            </div>
      ...
      {editingUser && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <RNCard className="w-full max-w-lg bg-[var(--bg-card)]">
            <h2 className="text-xl font-bold mb-4 text-[var(--text-primary)]">
              Edit User Details
            </h2>
