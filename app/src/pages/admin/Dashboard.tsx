...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      setLiveDuration(
        `${h}h ${m}m ${s}s` +
          (!todayStatus.clockOut && todayStatus.clockIn ? " (Running)" : "")
      );
    };
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <RNButton variant="outline" onClick={authService.logout}>
          Logout
        </RNButton>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat, index) => (
          <div
            key={index}
            className={`p-6 rounded-2xl border ${stat.className} transition-all hover:shadow-md bg-white dark:bg-[var(--bg-card)]`}
          >
            <div className="flex items-center justify-between mb-4">
              <div className="p-2 bg-white dark:bg-[var(--color-surface-800)] rounded-lg shadow-sm">
                {stat.icon}
              </div>
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-md transition-all ${
                      workFrom === "Office"
                        ...
                        : "text-[var(--text-muted)] hover:bg-gray-50 dark:hover:bg-slate-800"
                    }`}
                  >
                    Office
                  </button>
            ...
            <div className="flex gap-3">
              ...
              {!todayStatus || (todayStatus.clockOut && todayStatus.clockIn) ? (
                <RNButton
                  ...
                  className="px-8 py-3 bg-[var(--color-primary-600)] hover:bg-[var(--color-primary-700)] shadow-md hover:shadow-lg transition-all"
                >
        ...
        <Link to="/admin/users" className="block group">
          <RNCard className="h-full hover:border-[var(--color-primary-300)] transition-colors group-hover:shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-[var(--text-primary)] group-hover:text-[var(--color-primary-600)]">
                  User Management
                </h3>
                <p className="text-[var(--text-secondary)] text-sm mt-1">
                  Add, edit, or invite employees
                </p>
              </div>
      ...
      <div className="mt-8">
        ...
        <div className="border rounded-lg overflow-hidden bg-[var(--bg-card)] shadow-sm">
          <RNTable
            data={todaySummary}
            columns={[
              {
              ...
              },
              {
                ...
                cell: (row: any) => {
                  ...
                  else if (row.status === "Absent" || row.status === "Week Off")
                    variant = "destructive";
              ...
              },
              {
                ...
                accessorKey: "clockIn",
                cell: (row: any) =>
                  row.clockIn && row.status !== "Absent"
                    ? new Date(row.clockIn).toLocaleTimeString("en-US", {
                        hour: "numeric",
                        minute: "2-digit",
                        hour12: true,
                      })
              ...
              },
              {
                ...
                cell: (row: any) =>
                  row.clockOut && row.status !== "Absent"
                    ? new Date(row.clockOut).toLocaleTimeString("en-US", {
                        hour: "numeric",
                        minute: "2-digit",
                        hour12: true,
                      })
