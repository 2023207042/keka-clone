...
function PermissionRequestPage() {
  ...
  const columns = [
    ...
    { header: 'Admin Remarks', accessorKey: 'adminRemarks', cell: (row: any) => row.adminRemarks || '-' }
  ];
           ...
           <div className="lg:col-span-1">
                <RNCard title="New Request">
                    <form onSubmit={handleApply} className="space-y-4">
                       <RNSelect 
                         label="Request Type" 
                         options={[{label: 'Late Clock In', value: 'Late Clock In'}, {label: 'Early Check out', value: 'Early Check out'}]}
                         value={type}
                       ...
                       <RNTextarea label="Reason & Time" placeholder="e.g. Traffic delay, 10:30 AM arrival" value={reason} onChange={(e) => setReason(e.target.value)} required />
                       
                       {msg.text && <RNAlert variant={msg.type as any}>{msg.text}</RNAlert>}
                       
                       <RNButton type="submit" disabled={loading} className="w-full">{loading ? 'Submitting...' : 'Submit Request'}</RNButton>
                    </form>
                </RNCard>
           </div>
